...
if (!file.exists(path_to_db)){
  download_ted(dirname(path_to_db))
} else {
  update_ted(path_to_db, update = TRUE)
}

if (!check_ted(path_to_db)){
  stop("TED is not up-to-date")
}
...
plot_procedure_data <- long_procedure_data %>%
  ...
  mutate(
    value = fct_relevel(
      factor(value),
      "paraphrase", "exact","no", "none", "yes", "both","judgment", "exposure", 
